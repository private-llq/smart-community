	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (handler instanceof HandlerMethod) {
			...
			if(authAnnotation != null){
				...
				Object authTokenContent = UserUtils.getRedisToken("Sys:Auth", token);
...
//				}
		...
		if(!StringUtils.isEmpty(token)){
			...
			if(sysInfoVo != null){
				...
				request.setAttribute(UserUtils.USER_ROLE_ID, sysInfoVo.getRoleId());
				request.setAttribute(UserUtils.USER_TOKEN, sysInfoVo.getToken());
				return true;
			}
		}
		...
		throw new AdminException(JSYError.UNAUTHORIZED.getCode(), "登录过期");
	}
