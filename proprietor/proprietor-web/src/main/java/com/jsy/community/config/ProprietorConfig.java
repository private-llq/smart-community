...
public class ProprietorConfig {
     ...
     *
     * [使用方法]
     * 需要异步的方法上面增加：@Async(BusinessConst.PROPRIETOR_ASYNC_POOL) 注解
     * 建议异步使用线程执行的方法 放在Service 接口里面，像其他业务方法一样 实现 其实现方法
     * 而只在接口上面增加@Async(BusinessConst.PROPRIETOR_ASYNC_POOL) 注解
     ...
     */
    @Bean(BusinessConst.PROPRIETOR_ASYNC_POOL)
    public ThreadPoolTaskExecutor proprietorTaskActuator() {
        ...
        //线程前缀名称
        executor.setThreadNamePrefix("Proprietor-Async-Thread");
        //配置拒绝策略
